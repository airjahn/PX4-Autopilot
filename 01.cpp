...
private:
    double former_x=0.0,former_y=0.0,former_z=0.0,former_yaw=0.0;//用于保存上一状态的 x y z 坐标
 ...
 * 					为主要修改对象
 ************************************************************/
    former_x = _vehicle_local_position.x;//位置坐标初始化
	former_y = _vehicle_local_position.y;
	former_z = _vehicle_local_position.z;
	former_yaw = 0.0;
    ...
    while(1)//此循环为业务逻辑
    {
        if(flag==0)//升至1.5米
        {
            ...
            _pos_sp_triplet.current.z=double(_vehicle_local_position.z)-0.5;
            
            former_x = _pos_sp_triplet.current.x;//位置坐标保存
		    former_y = _pos_sp_triplet.current.y;
			former_z = _pos_sp_triplet.current.z;
            ...
            _position_setpoint_triplet_pub.publish(_pos_sp_triplet);
             ...
             mavlink_log_critical(&mavlink_log_pub, "起飞");
        }
     
        //if(flag==1&&(hrt_absolute_time()-time_tick>5000000))
        //{
            //flag++;
            //memset(&_pos_sp_triplet, 0, sizeof(position_setpoint_triplet_s));//全部清0
            //_pos_sp_triplet.current.valid = true;
            //_pos_sp_triplet.current.position_valid = true;
            //_pos_sp_triplet.current.x=former_x+0.4;
            //_pos_sp_triplet.current.y=former_y;
            //_pos_sp_triplet.current.z = former_z;
            
            //former_x = _pos_sp_triplet.current.x;//前一状态位置坐标保存
		    //former_y = _pos_sp_triplet.current.y;
			//former_z = _pos_sp_triplet.current.z;
            ...
            //_position_setpoint_triplet_pub.publish(_pos_sp_triplet);
             //time_tick=hrt_absolute_time();
             //mavlink_log_critical(&mavlink_log_pub, "x正方向");
        //}
       //if(flag==2&&(hrt_absolute_time()-time_tick>3000000))
        //{
            //flag++;
            //memset(&_pos_sp_triplet, 0, sizeof(position_setpoint_triplet_s));//全部清0
            //_pos_sp_triplet.current.valid = true;
            //_pos_sp_triplet.current.position_valid = true;
            //_pos_sp_triplet.current.x=former_x;
            //_pos_sp_triplet.current.y =former_y+0.4;
            //_pos_sp_triplet.current.z = former_z;
            //former_x = _pos_sp_triplet.current.x;//前一状态位置坐标保存
		    //former_y = _pos_sp_triplet.current.y;
			//former_z = _pos_sp_triplet.current.z;
            ...
            //_position_setpoint_triplet_pub.publish(_pos_sp_triplet);
             //time_tick=hrt_absolute_time();
             //mavlink_log_critical(&mavlink_log_pub, "y正方向");
		 //}
